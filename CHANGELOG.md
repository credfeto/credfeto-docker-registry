...
- Fixed `cleanup` script to run `podman exec` as the `registry` user with correct `XDG_RUNTIME_DIR`; corrected container name from `registry_docker` (hostname) to `registry` (container name)
### Removed
### Deployment Changes
